...
subtitle: "Workshop for the Edinburgh Centre for Data, Culture & Society"
...
**Welcome to the CDCS workshop on Visualizing Your Data Using R.**

This worksheet is structured the following way:

- Troubleshooting: refer here if you get an error.
- A number of sections about various plots, (some of) which we will go though together.
- A self-guided part in the end - you're free to select your own task.
- An appendix with some extras like importing your own data.
- Press ALT+O to collapse all sections (gives a nice overview of the worksheet)
- Click the little blue double arrow next to a section title to re-expand; press ALT+SHIFT+O to expand all.
...
# The command above, when executed (what you just did), printed the text in the console below. Also, this here is a comment. 
# Commented parts of the script (anything after a # ) are not executed. 
# This R Markdown file has both code blocks (gray background) and regular text (white background).
...
### Exercises:
- compare this plot with your neighbor. Do they look the same? Why/why not?
...
First let's load a package, and set up a dataset that we will be using for a number of exercises down the line.
```{r loaddata}
library(languageR) # load the necessary package; cursor on (anywhere on) the line, press CTRL/CMD + ENTER
...
## Basic plotting using R's default graphics package
...
# So we loaded a package that came with a dataset. 
...
boxplot(eng$RTnaming ~ eng$AgeSubject, 
        main="Reaction time by age", ylab="Reaction time", 
...
# Two numeric variables, and regression:
plot(WrittenFrequency ~ Familiarity,  data=eng, col="black", pch=20)
grid(col=rgb(0,0,0,0.2), lty=1)
abline( lm(WrittenFrequency ~ Familiarity,  data=eng), col=rgb(0,0,0,0.2), lwd=4 )
...
The `rgb(red, green, blue, alpha)` function allows making custom colors; `alpha` controls transparency. Possible values range between 0 and 1 by default. 
You can also specify many colors by name (e.g., "red", "navy"). 
Another good way to use colors is to use ready-made palettes.

```{r colors, echo=F}
library(RColorBrewer) # load a package for more palettes
plot(1:5, pch=16, cex=18, col=1:5)                          # colors as numbers from the default colorspace
plot(1:5, pch=16, cex=18, col=c("red", "tomato", "orange", "gold", "yellow")) # named colors
plot(1:5, pch=16, cex=18, col=rgb(1, seq(0,0.8,0.2), seq(0,0.4,0.1) ,0.8) )    # rgb specification
plot(1:5, pch=16, cex=18, col=gray.colors(5))                                 # one of the default palettes
plot(1:5, pch=16, cex=18, col = brewer.pal(5, "Spectral") )                   # a palette from RColorBrewer

# jumping ahead: color scales in ggplot:
library(ggplot2)      # load ggplot2
ggplot() + aes(x=1:5, y=1:5, color=1:5) + 
  geom_point(size = 40) + 
  theme_classic() + 
  scale_color_viridis_c() # use the viridis palette
...
library(ggplot2)    # load ggplot2
# (if doing this exercise at home later, make sure to load the libraryR package and do the eng=english[... subsetting above first)
...
- Since the data is conformed to a few integer values, a scatterplot is hard to read as is. Add the following parameter to geom_point(): ` position = position_jitter(width=0.2,height=0.2) `
...
s2 = stringdistmatrix(unique(words), useNames = T ) %>% as.matrix() %>% melt()

# What's an edit distance? It just quantifies how many letters have to be edited (deleted/added/replaced) to turn one word into another word:
...
# solution: plot_grid(ggplot() + aes(x) + geom_histogram(binwidth = 3),ggplot() + aes(x) +  geom_density(adjust = 0.3) + geom_rug(color=rgb(0,0,0,0.2)))
...
# There are more ways of combining these using additional packages:
library(ggridges)
plot_grid(
ggplot() + aes(x=x, y=1) +
...
plot_grid(ncol=3,
  ggplot(d) + aes(x=time, y=y ) + geom_line(col="orange",size=1.5) +ylim(0,5) + labs(subtitle="series 1",title="",y="") ,
  ggplot(d) + aes(x=time, y=y ) + geom_line(col="red", size=1.5) + labs(subtitle="series 2",title="",y="") ,
  ggplot(d) + aes(x=time, y=y ) + geom_line(col="darkblue", size=1.5) +ylim(0,20) + labs(subtitle="series 3",title="",y="") ) + draw_figure_label("Which series depicts the most drastic change over time?")
...
# Here's something also pretty useless: a surface plot of a volcano, with the height coded with color:
...
# Making slides with integrated plots using RMarkdown
...
plot_grid(cloud, gov)  # arrange and show ggplots
```
...
# Time to put your newly-acquired skills to use

We only went through some of the sections in this worksheet. For the last part of the workshop, it's up to you what to do. You could go back to the plots we did together, and continue playing around with them, or crack open a new section and have a go. Or you could import your own data (see the Appendix below) and make some cool plots using some of the code snippets in this worksheet.

Also, feel free to showcase your creations on Twitter ;)
Use the Export button above the plots pane and save as image (or refer to the RMarkdown sections above on how to save as a webpage). If you do post plots based on code from this worksheet, kindly mention the following accounts in the tweet: ` @andreskarjus @aRtofdataviz `(former: me, latter: my dataviz workshops account). You might also want to use the #rstats and #dataviz hashtags. 
If your plot goes horribly wrong and ends up looking like a piece of modern art, tag @accidental__aRt and they'll likely retweet.
