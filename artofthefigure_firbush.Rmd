...
All the inputs to the function go inside the ( ) brackets. In the above case, the text is the input to the `print()` function. All text, or "strings", must be within quotes.
...
# But a very simple trick is to just remove all short words:
...
# once you are done with wordcloud, execute this to clear the plotting area parameters:
dev.off()
```
