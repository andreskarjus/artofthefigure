---
title: "aRt of the Figure, Firbush edition"
...
- We will be using plot() and a few of its cousins, but their parameters (like `col` for color) usually all have the same or similar names.
...
```{r, echo=F, eval=F}
# Tip: feel free to click the downward triangle on the left side bar (the one with line numbers) to temporarily hide any section.
...
Also, if you've been scrolling left and right in the script window to read the code, turn on text wrapping ASAP: on the menu bar above, go to Tools -> Global Options -> Code (on the left) -> tick "Soft-wrap R source files".
...
- `fun2( fun1(do, something), parameters ) # fun1 is evaluated first, and its output becomes the input for fun2.`
...
# install.packages("quanteda") # install this package to make the last example works
...
```{r, eval=F, echo=F}
# This block is for installing packages, leave eval=F
...
# Bonus: You may notice that in the visNetwork help files, the examples use the magrittr package's %>% pipe notation. The example here does not, in order to keep things simple, but feel free to explore magrittr, it makes writing sequences such as the one below much more elegant.
...
uk = getData("GADM", country = "United Kingdom", level = 2) # download UK map (needs the raster package to be loaded; need internet connection)
