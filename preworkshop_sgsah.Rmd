...
```{r, eval=F}
p=c("magrittr","ggplot2","cowplot","ggbeeswarm","reshape2","dplyr","RColorBrewer","viridis","plotly","languageR","quanteda","corrplot","rmarkdown","stringdist","flexdashboard","sf","jsonlite","rworldmap"); install.packages(p); x=p%in%rownames(installed.packages());if(all(x)){print("Packages installed successfully!")}else{print(paste("Failed to install:", paste(p[!x]), ", try again and make sure you have internet connection."))};rm(x,p)
```
